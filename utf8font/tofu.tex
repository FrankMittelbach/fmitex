...
\personalURL{https://ctan.org/pkg/utf8font}
...
\section{Tofu through the years}

Tofu is not just an essential ingredient for many Asian dishes, it is
also the nick name for little squeres that are produced by many
bowsers when they got asked to render a character for which they glyph
available.

Especially in the early days of of the World Wide Web,
websites in foreign langages (from the perspective of your computer)
...
\begin{quote}
  ...
  souffl\tofu{}s?
\end{quote}
Over the years the situation with browsers improved, but even nowadays
you may find tofu-littered sites or perhaps worse those that think they
can show you the glyphs but render the wrong ones.

While with browswers you may accept a certain imperfection in the
rendering, tofu in printed material is quite inacceptable and
...
so that you can apply some corrective actions. In the remainder of
...
In the early days of \TeX{} the use of fonts was easy as you could use
any font you wanted as long as it was called Computer Modern.

In other words there was essentially only one set of fonts available
and the glyphs it contained and how to address them was described in
the \TeX{}book~\cite{}. Furthermore, all fonts only contained 128
glyphs, i.e., essentially the base Latin characters, a few accents to
construct diacritical characters using the \cs{accent} primitive and a
few other symbols such as \textdagger, \$ and so forth to be accessed
through command names.
...
So when \TeX{} got more and more polular outside the English-speaking
world there was considerable preasure on Don Knuth (largely by
...
way) \TeX{} development and produce \TeX~3. This version of \TeX{} was
then able to deal with more than one language within a document (use
multiple hyphenation patterns) support 8-bit input output (that is 256
characters in a font).

While this enabled use of different input codepages as common in in
...
Depending on the codepage used in the computer when writing a document
the same keyboard character might be associated with a different
number (between 0 and 255) and that number had to be mapped to the
right slot in a font to produce the glyph that was originally
...
Scheme (NFSS)~\cite{} and in particular withith the packages
\LPack{inputenc} (for managing input in different codepages and
...
and where each glyph is stored in the font. Thus, if you had two
...
by providing the necesary font reencodings through the virtual font
mechanism or through reencodings done by device drivers such as
...
essentially every font available was evailable in \texttt{T1} encoding
...
the quite common \texttt{latin1} input encoding.\footnote{For example,
  ...
  character.}  Years later, ommitting the input encoding declaration
...
declaration.\footnote{The remedy for such old documents is then to
  either add the missing declaration or reencode the old source and
  store them in UTF-8.}
...
that occured because a text was written under the assumption of one
